...
{
    public class UpdateBrandCommand : RequestContext<BaseResponse>
    {
        public long Id { get; private set; }
        public string Name { get; private set; }
        public ClaimsPrincipal ClaimsPrincipal { get; }

        public UpdateBrandCommand(long id, BrandRequest request, ClaimsPrincipal claimsPrincipal)
        {
            Id = id;
    ...
    public class UpdateBrandCommandHandler : IRequestHandler<UpdateBrandCommand, BaseResponse>, IDisposable
    {
        private readonly IResult _result;
        private readonly IRepositoryManager _repositoryManager;
        private readonly IIdentityService _identityService;

        public UpdateBrandCommandHandler(IResult result, IRepositoryManager repositoryManager, IIdentityService identityService)
        {
        ...
        {
            var brand = await _repositoryManager.BrandRepo.SingleOrDefaultAsync(x => x.Id == request.Id, withTracking: true);
            if (brand == null)
            {
            ...
            brand.Update(request.Name, userIdResult.Payload.ApplicationUserId);
